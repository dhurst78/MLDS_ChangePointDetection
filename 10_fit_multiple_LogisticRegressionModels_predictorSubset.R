...
# - Calculate all 12 predictors, EXCLUDING momentum 2,3,4
...
pdf(file="AuC_2x2_scatter_2.pdf", width = 10.0, height = 6.0)
...
plot(Mquintile$CPcount.x,Mquintile$CPcount.y,pch=16,col=6,xlim = c(5,1500),ylim = c(5,1500), log='xy',main='No of Change Points', xlab='Including Interactions', ylab='Excluding Interactions')
...
pdf(file="predictor_boxplots_combined_2.pdf", width = 10, height = 6.)
...
##################################################################
##
sum(abs(diff(TRAINorig$P)))


results_triplets[results_triplets$AUC>0.7 & results_triplets$CPcount < 40, ]


Mpairs[Mpairs$AUC.y>0.7 & Mpairs$CPcount.y < 40, ]

Mtriplets[Mtriplets$AUC.y>0.7 & Mtriplets$CPcount.y < 40, ]

Mquads[Mquads$AUC.y>0.7 & Mquads$CPcount.y < 40, ]
Mquintile[Mquintile$AUC.y>0.7 & Mquintile$CPcount.y < 40, ]
...
plot( Mpairs$CPcount.x, Mpairs$CPcount.y)#,xlim = c(0,100),ylim = c(0,100))
abline(0,1)